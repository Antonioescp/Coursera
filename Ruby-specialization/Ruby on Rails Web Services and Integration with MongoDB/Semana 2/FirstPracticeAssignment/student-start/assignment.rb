...
class Solution
  ...
  def concat_names
    @coll.find.aggregate([{:$project => {_id: 0, name: {:$concat => ['$first_name', ' ', '$last_name']}, number: 1}}])
  end
  ...
  def group_last_names_set
    @coll.find.aggregate([{ :$group => {_id: { age: '$group', gender: '$gender'}, last_names: {:$addToSet => '$last_name'}} }, {:$sort => {runners: 1}}])
  end
  ...
  def groups_faster_than criteria_time
    @coll.find.aggregate([{:$match => {secs: {:$lt => criteria_time}}}, {:$group => {_id: {age: '$group', gender: '$gender'}, runners: {:$push => '$first_name'}, fastest_time: {:$min => '$secs'}}}])
  end
